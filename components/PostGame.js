...
const PostGame = ({ t, i18n, gameState, socket }) => {
	...
	const successPlayers = successVote
		? players.filter((p) => p.goodVote).map((p) => p.name)
		: [];
	const strSuccessPlayers =
		successPlayers.length > 1
			? successPlayers.slice(0, -1).join(", ") +
			  " and " +
			  successPlayers.slice(-1)
			: successPlayers[0];
	const winners = players.filter((p) => p.gameWon).map((p) => p.name);
	const strWinners =
		winners.length > 1
			? winners.slice(0, -1).join(", ") + " and " + winners.slice(-1)
			: winners[0];
			...
			{successVote ? (
				<div className="success-vote-container">
					<div className="success-vote-text">
						{isMafia ? "You were outed!" : "The mafia was outed!"} <br />
						{isAllMafia ? (
							"Everyone was mafia!"
						) : (
							<>
								{strSuccessPlayers} successfully voted for <b>{mafia.name}</b>!
								(+{settings.townGuessPoints})
							</>
						)}
					</div>
				</div>
			...
			{winners.length > 0 && (
				<div className="winners-container">
					<div className="winners-text">
						<br />
						{strWinners} won the RL Match! (+{settings.gameWinPoints} to
						non-mafia)
					</div>
				</div>
			)}
