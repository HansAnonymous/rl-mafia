...
const Game = ({ t, loading }) => {
	...
	useEffect(() => {
		...
		console.log(gameCode + " game found");
	...
	const showVoting = !showNameEntry && status === "voting";
	const showPostGame = !showNameEntry && status === "postgame";
		...
		<>
			...
			{!showLoading && (
				<>
					...
					{showVoting && <Voting gameState={gameState} socket={socket} />}
					{showPostGame && <PostGame gameState={gameState} socket={socket} />}
				</>
			)}
		</>
	);
